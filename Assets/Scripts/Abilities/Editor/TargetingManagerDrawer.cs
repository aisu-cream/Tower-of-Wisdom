...
[CustomPropertyDrawer(typeof(TargetingManager), true)]
public class TargetingManagerDrawer : PropertyDrawer {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        ...
        if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Targeting Manager"), FocusType.Keyboard)) {
            var menu = new GenericMenu();

            if (typeMap == null || typeMap.Count == 0) {
                menu.AddDisabledItem(new GUIContent("No Targeting Manager Available"));
                menu.ShowAsContext();
                return;
            }
    ...
    static void BuildTypeMap() {
        var baseType = typeof(TargetingManager);
