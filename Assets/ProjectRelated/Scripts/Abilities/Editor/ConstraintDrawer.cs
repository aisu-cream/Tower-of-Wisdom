...
[CustomPropertyDrawer(typeof(IConstraint), true)]
public class ConstraintDrawer : PropertyDrawer {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        ...
        if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Constraint Type"), FocusType.Keyboard)) {
            var menu = new GenericMenu();
           
            if (typeMap == null || typeMap.Count == 0) {
                menu.AddDisabledItem(new GUIContent("No Constraints Available"));
                menu.ShowAsContext();
                return;
            }
    ...
    static void BuildTypeMap() {
        var baseType = typeof(IConstraint);
